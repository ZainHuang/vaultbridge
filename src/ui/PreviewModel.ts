...
export function canExecutePreview(preview: Pick<SyncPreview, 'canExecute' | 'mode' | 'plan'>): boolean {
  return preview.canExecute && preview.mode !== 'BLOCKED' && !preview.plan.hasConflicts && !repositoryBlocked(preview.plan);
}
export function repositoryBlocked(plan: Pick<ThreeWayPlan, 'status'>): boolean {
